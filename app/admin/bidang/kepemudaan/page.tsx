...
export default function KepemudaanAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const finalImageUrl = await uploadFileBase64(uploadedImageBase64, `kepemudaan-card-${Date.now()}.webp`, 'kepemudaan');
      ...
      if (modalType === 'add') {
        ...
        success = await setKepemudaanCards([newItem, ...kepemudaanCards]);
        ...
        success = await setKepemudaanCards(kepemudaanCards.map(c => c.id === editingItem.id ? updatedItem : c));
  ...
  const handleUpdateBottomCard = async (e: React.FormEvent) => {
    ...
    try {
      const finalBannerUrl = await uploadFileBase64(bannerImageUrl, `kepemudaan-banner-${Date.now()}.webp`, 'kepemudaan');
      const existingBanner = bidangBottomCards.find(c => c.id === 'kepemudaan');
